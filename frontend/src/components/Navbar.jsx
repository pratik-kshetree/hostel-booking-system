...
if(mode==="dark"){
  ...
  useEffect(()=>{
    ...
    //using variable navitem so that we can use further like this {navitems}
    const navitems=(
        <>
            <li><NavLink to="/" className={({isActive})=>(isActive ? "text-blue-500" :"" )}>
            Home</NavLink></li>
            <li><NavLink to="/room" className={({isActive})=>(isActive ? "text-blue-500" : " ")}>
            Rooms</NavLink></li>
  ...
  return (
    <div className={`dark:bg-slate-900 dark:text-white fixed top-0 left-0 right-0 z-50 border-b transition-all duration-300 ease-in-out${
      sticky?" bg-base-300  shadow-md ":"bg-transparent dark:bg-slate-700 dark:text-white"
    }`}>
