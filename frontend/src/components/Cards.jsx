...
function Cards({ item, allowBooking }) {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      //   toast.error(error.response.data.message || "Sorry,All Room Are Booked")
      // }
      // else{
  ...
  return (
    <>
      <div className='mb-4 p-3'>
        <div className="card bg-base-100 w-full shadow-2xl hover:scale-105 duration-200 dark:bg-slate-900 dark:text-white border h-full flex flex-col">
          <figure>
            {(() => {
              const backendBase = "http://localhost:4001"; // adjust if your backend runs on a different port
              const imgPath = item?.image || "";
              const imageSrc = imgPath.startsWith("/uploads") ? `${backendBase}${imgPath}` : imgPath;
          ...
          <div className="card-body flex-1 flex flex-col justify-between">
            ...
            <div className="tooltip" data-tip={item.title}>
              <p className="line-clamp-2 h-12 overflow-hidden cursor-pointer">{item.title}</p>
            </div>
